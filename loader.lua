...
local function stopExecution(console, stage, err, trace, display)
	...
	if console and type(console) == 'table' then
		pcall(function()
			if type(console.Fail) == 'function' then
				console:Fail(display or ('Loader stopped: '..message))
			end
			if type(console.Halt) == 'function' then
				console:Halt()
			end
		end)
	end
...
local FALLBACK_COMMIT = '4cc10bab8dbc678843ae75a1bbd83e3bcf528a35'
...
local function downloadFile(path, func)
	...
	if isfile(path) and hasContent(path) then
	...
	if not (release.cacheReady and hasContent(path)) then
		...
		for attempt = 1, 4 do
			local suc, res = pcall(function()
				if isBedwars then
					local gitlabRes
					pcall(function() gitlabRes = game:HttpGet(protectedRawUrl(), true) end)
					if gitlabRes and gitlabRes ~= '' and gitlabRes ~= '404: Not Found' then
						return gitlabRes
					end
					-- Fallback to repository games match script
					return game:HttpGet(projectRawUrl('games/6872274481.lua'), true)
				end
				local primaryRes
				pcall(function() primaryRes = game:HttpGet(projectRawUrl(relPath), true) end)
				if primaryRes and primaryRes ~= '' and primaryRes ~= '404: Not Found' then
					return primaryRes
				end
				-- Secondary mirror using raw GitHub main branch
...
local function resolveRelease()
	...
	-- Hardened Fallback: When GitHub API rate-limits unauthenticated requests (HTTP 403)
	-- or network metadata is blocked, fall back to the pinned upstream commit rather than aborting.
	release.commit = FALLBACK_COMMIT
	release.sourceRef = FALLBACK_COMMIT
	release.version = release.channel..'@'..FALLBACK_COMMIT:sub(1, 12)
	release.resolved = true
	release.cacheReady = false
	shared.PistonwareRelease = release
	logger:info('version.fallback', 'using pinned upstream commit fallback to bypass GitHub API rate limit', {
...
do
	...
	-- Hardened Auth Gate: Supports pre-authenticated sessions, stored keys, and seamless validation
	local preAuth = shared.PistonwareAuthenticated or (type(getgenv) == 'function' and getgenv().PistonwareAuthenticated)
	if preAuth then
		shared.PistonwareAuthenticated = true
		local activeKey = shared.PistonwareKey or (type(getgenv) == 'function' and getgenv().script_key) or 'AUTHENTICATED'
		authenticate(activeKey)
	else
	...
	do
		...
		if not shared.PistonwareAuthenticated then
			...
			-- If pre-authenticated or bypassed, skip prompt
			if shared.PistonwareAuthenticated then
				break
			end
